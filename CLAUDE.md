...
**特殊シリーズ「劇場版・OV作品」**: TVシリーズ本編には登場しない劇場版・OVA・配信作品限定のオリジナル怪獣・宇宙人をまとめた受け皿シリーズ。`episode`には話数の代わりに作品タイトル（例:「ウルトラマンティガ THE FINAL ODYSSEY」）を入れる。単一の放送年を持たないため`SERIES_ORDER`/`SERIES_YEAR`には登録せず、`index.html`のシリーズ一覧は`kaiju.json`から動的に生成されるため登録なしでも表示される（年代順ソートの末尾に配置）。
